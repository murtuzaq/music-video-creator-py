...
class AssetPanel:
    ...
    def add_asset(self, path: str, asset_type: str) -> bool:
        """Append an asset; returns False (and skips) if the path is already listed."""
    ...
    def clear(self):
        """Destroy all rows and reset thumbnail cache."""
    ...
    def _add_row(self, entry: dict):
        ...
        row = tk.Frame(self._inner, bg="#333", pady=4, padx=4)
        ...
        if asset_type == "image" and _PIL:
            ...
            if photo:
                lbl = tk.Label(row, image=photo, bg="#333")
        ...
        tk.Button(row, text="×", command=_remove,
                  bg="#333", fg="#999", activebackground="#555",
                  activeforeground="white", relief=tk.FLAT,
                  font=("Helvetica", 12), padx=2, pady=0,
                  cursor="hand2").pack(side=tk.RIGHT, anchor="n")

    def _icon(self, parent, text: str, color: str):
        tk.Label(parent, text=text, bg="#333", fg=color,
                 font=("Helvetica", 14), width=3).pack(side=tk.LEFT, padx=(0, 5))
